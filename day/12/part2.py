...
def DFS(nodes, repeats=0, start=Node('start'), goal=Node('end'), seen=None):
	if seen is None:
		seen = {start}
	...
	if start == goal:
		...
		for node in nodes[start]:
			new_repeats = repeats - (node in seen and not node.big)
			if node.name == 'start' or new_repeats < 0: continue
			
			for path in DFS(nodes, new_repeats, node, goal, seen | {node}):
				yield [start] + path
...
def main(args):
	...
	count = 0
	for path in DFS(nodes, repeats=1):
