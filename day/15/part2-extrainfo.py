...
def calculate_repeat_risk(orig, offsets):
	return ((orig + offsets - 1) % 9) + 1

def main(args):
	tile = Map([[int(c) for c in line if c != '\n'] for line in args.input])
	map = Map([[calculate_repeat_risk(tile[row, col], h+v)
	            for h in range(args.repeats) for col in range(tile.cols)]
	           for v in range(args.repeats) for row in range(tile.cols)])
	paths = djikstra(map)
	end = map.rows-1, map.cols-1
	info(f'Final path risk: {paths[end].pathrisk}')
	debug(f'Final path: {pformat(list(iterpath(paths, end)))}')
	backtracks = 0
	for first, second in itertools.pairwise(iterpath(paths, end)):
		if second[0] < first[0]:
			debug(f"Backtracking from {first} to {second}")
			backtracks += 1
	info(f'This path involves {backtracks} instances of going up or left')

if __name__ == '__main__':
	parser = aoc.get_argparser()
	parser.add_argument('-r', '--repeats', type=int, default=1,
	                    help="Number of copies of the original tile (with increments) in each direction")
